...
namespace OnlineShopping.Utility
{
    public class UploadContent
    {
        ...
        public void UploadImage(HttpPostedFileBase originalImage, string imagePrefix, string rootPath, HttpServerUtilityBase server, GenericUnitOfWork _unitOfWork, int memberId, int productId = 0, int categoryId = 0)
        {
            ...
            for (int i = originalImage.FileName.Length - 4; i >= 0; i--)
            {
                if (originalImage.FileName[i] == '\\')
                {
                    poz = i;
                    break;
                }
            }
            string filename =  originalImage.FileName.Substring(poz + 1);

            originalImage.SaveAs(server.MapPath("~/" + rootPath + "Original/" + imagePrefix + filename));

            // Large size for service banner image
            if (productId != 0)
            {
                WebImage img1 = new WebImage(server.MapPath("~/" + rootPath + "Original/" + imagePrefix + filename));
                img1.Resize(849, 320);
                ...
                img1.Save(Path.Combine(server.MapPath("~/" + rootPath + "Large/" + imagePrefix + filename)));
            }

            WebImage img2 = new WebImage(server.MapPath("~/" + rootPath + "Original/" + imagePrefix + filename));
            img2.Resize(274, 175);
            ...
            img2.Save(Path.Combine(server.MapPath("~/" + rootPath + "Medium/" + imagePrefix + filename)));

            WebImage img3 = new WebImage(server.MapPath("~/" + rootPath + "Original/" + imagePrefix + filename));
            img3.Resize(68, 68);
            ...
            img3.Save(Path.Combine(server.MapPath("~/" + rootPath + "Small/" + imagePrefix + filename)));

            System.IO.File.Delete(server.MapPath("~/" + rootPath + "Original/" + imagePrefix + filename));
        }
